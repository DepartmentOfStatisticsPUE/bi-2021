...
dane %>% 
  ...
  mutate(ind_type = case_when(ind_type == "All Individuals"  ~ "Ogółem",
         ...
         ind_type = factor(ind_type, levels = c("Ogółem", "16-24", "25-34", "35-44", "45-54", "55-74"), 
                           ordered = TRUE)) %>%
  select(ind_type, time, values, geo) %>%
  ggplot(data = .,
         aes(x = time,
             y = values,
             group = geo)) + 
...
ggsave(filename = "../raporty/wyklad-1/6-eurostat-wiek.png", width = 8, height = 4)
```
...
ggsave(filename = "../raporty/wyklad-1/6-eurostat-sasiedzi.png", width = 8, height = 4)
```
...
ggsave(filename = "../raporty/wyklad-1/6-eurostat-inne.png", width = 8, height = 4)
```
